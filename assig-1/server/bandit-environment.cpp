...
*/
bool setRunParameters(int argc, char *argv[], int &numArms, int &randomSeed, unsigned long int &horizon, unsigned long int &explorationHorizon, string &banditFile, int &port){
...
int main(int argc, char *argv[]){
  ...
  if(!(setRunParameters(argc, argv, numArms, randomSeed, horizon, explorationHorizon, banditFile, port))){
  ...
  cout << "Horizon: " << horizon << "\n";
  cout << "Exploration horizon: " << explorationHorizon << "\n";
  ...
  */
  ...
  double regretAtExplorationHorizon = 0;
  for(unsigned int i = 0; i < horizon; i++){

    if(i == explorationHorizon){
      regretAtExplorationHorizon = bandit->getRegret();
    }
  ...
  double regret = bandit->getRegret() - regretAtExplorationHorizon;
  cout << "Regret = " << regret << "\n";
  cout << "Terminating.\n";
